...
Structured preview tools are implemented for `product.create.preview`, `product.update.preview`, `product.media.update.preview`, `product.importFromUserUrl.preview`, `inventory.setQuantity.preview`, `inventory.adjustQuantity.preview`, `inventory.moveQuantity.preview`, `inventory.transfer.preview`, `page.create.preview`, and `collection.create.preview`. These previews summarize user-provided inputs, validate required fields, create audit entries, and do not call Shopify write APIs or perform mutations. `product.create.preview`, `inventory.setQuantity.preview`, `inventory.adjustQuantity.preview`, `inventory.moveQuantity.preview`, `page.create.preview`, and `collection.create.preview` also return a safe `executeRequest` helper so AI hosts can prepare the matching execute call for user review without manually copying every hash and payload field. `inventory.transfer.preview` is preview-only and does not return `executeRequest` because transfer execute is not implemented.
...
`inventory.transfer.preview` is a preview-only planning tool for future inter-location transfer workflows. It requires one explicit inventory item ID, a source location ID, a destination location ID, a positive quantity, and an explicit reason. It stores a safe reviewable plan, performs no Shopify write, and has no execute tool yet.
