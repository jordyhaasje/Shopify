...
Structured preview tools are implemented for `product.create.preview`, `product.update.preview`, `product.media.update.preview`, `product.importFromUserUrl.preview`, `inventory.setQuantity.preview`, `page.create.preview`, and `collection.create.preview`. These previews summarize user-provided inputs, validate required fields, create audit entries, and do not call Shopify write APIs or perform mutations. `product.create.preview`, `inventory.setQuantity.preview`, `page.create.preview`, and `collection.create.preview` also return a safe `executeRequest` helper so AI hosts can prepare the matching execute call for user review without manually copying every hash and payload field.
