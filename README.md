...
Real read-only Shopify MCP tools are implemented for `shopify.capabilities.check`, `order.find`, `order.get`, `customer.find`, `tracking.get`, `product.get`, `inventory.lookup`, and `inventory.locationLookup`.

Structured preview tools are implemented for `product.create.preview`, `product.update.preview`, `product.media.update.preview`, `product.importFromUserUrl.preview`, `inventory.setQuantity.preview`, `inventory.adjustQuantity.preview`, `inventory.moveQuantity.preview`, `inventory.transfer.preview`, `inventory.transfer.addItems.preview`, `inventory.transfer.removeItems.preview`, `inventory.transfer.markReady.preview`, `inventory.transfer.cancel.preview`, `inventory.transfer.ship.preview`, `inventory.transfer.receive.preview`, `page.create.preview`, and `collection.create.preview`. These previews summarize user-provided inputs, validate required fields, create audit entries, and do not call Shopify write APIs or perform mutations. `product.create.preview`, add-only `product.media.update.preview`, `inventory.setQuantity.preview`, `inventory.adjustQuantity.preview`, `inventory.moveQuantity.preview`, `inventory.transfer.preview`, `inventory.transfer.addItems.preview`, `inventory.transfer.markReady.preview`, `inventory.transfer.cancel.preview`, `inventory.transfer.ship.preview`, `inventory.transfer.receive.preview`, `page.create.preview`, and `collection.create.preview` also return a safe `executeRequest` helper so AI hosts can prepare the matching execute call for user review without manually copying every hash and payload field. `inventory.transfer.removeItems.preview` is preview-only and intentionally does not return `executeRequest` until a future execute path is implemented.

`bulk.preview` is implemented as a safe generic preview surface for explicit batch changes. It stores a preview binding and audit entry, but returns summarized value shapes only: value types, string lengths, object keys, array counts, changed non-secret keys, and redaction markers. It must not echo raw before/after payload values, secrets, customer/order data, or raw Shopify nodes. `bulk.execute` remains a fail-closed placeholder, so `bulk.preview` does not return an `executeRequest` helper.
...
`product.media.update.execute` is the fourth limited real Shopify write tool. It only attempts `productUpdate(product:, media:)` when read-only mode is disabled, local granted scopes include `write_products`, a stored add-only `product.media.update.preview` record exists, the preview is not expired, the reviewed payload hashes back to the stored preview, the target/tool/hash binding matches, and `confirmed: true` is present. It supports only adding user-provided HTTP(S) media URLs or staged upload URLs to one explicit product ID, with media content type defaulting to `IMAGE` when the preview supplied URL/image input and optional alt text. Handle-only previews fail closed unless the stored preview contains a safe product ID. It does not implement media update/delete/reorder/replacement, staged upload orchestration, file library workflows, variant-media attachment, product discovery, or bulk media operations.

`collection.create.execute` is the fifth limited real Shopify write tool. It only attempts the Shopify collection create mutation when read-only mode is disabled, local granted scopes include `write_products`, a stored `collection.create.preview` record exists, the preview is not expired, the reviewed payload hashes back to the stored preview, the target/tool/hash binding matches, and `confirmed: true` is present. It supports only custom collection creation from a reviewed title, optional handle, and explicit product IDs in the stored preview. It does not implement rule-based or smart collections, publishing, SEO, metafields, collection media, collection update/delete, navigation, product discovery, or bulk operations.
...
`inventory.locationLookup` is a read-only helper for finding explicit location IDs needed by reviewed inventory workflows. It accepts one explicit location ID, location name, or location query, then returns compact location ID/name/active summaries. It does not write, does not run during execute as hidden discovery, and does not return raw Shopify nodes.

`inventory.setQuantity.execute` is the sixth limited real Shopify write tool. It only attempts `inventorySetQuantities` when read-only mode is disabled, local granted scopes include `write_inventory`, a stored `inventory.setQuantity.preview` record exists, the preview is not expired, the reviewed payload hashes back to the stored preview, the target/tool/hash binding matches, and `confirmed: true` is present. It supports one explicit inventory item ID, one explicit location ID, quantity name `available`, one non-negative integer quantity, an explicit reason, and compare-and-set through `compareQuantity` unless `ignoreCompareQuantity: true` was explicitly previewed. Execute uses only the stored preview as the source of truth; it does not perform lookup/discovery during execute and it does not implement bulk inventory, state moves, product updates, or location management.

`inventory.adjustQuantity.execute` is the seventh limited real Shopify write tool. It only attempts `inventoryAdjustQuantities` when read-only mode is disabled, local granted scopes include `write_inventory`, a stored `inventory.adjustQuantity.preview` record exists, the preview is not expired, the reviewed payload hashes back to the stored preview, the target/tool/hash binding matches, and `confirmed: true` is present. It supports one explicit inventory item ID, one explicit location ID, quantity name `available`, one non-zero integer delta, an explicit reason, and optional reference document URI. It does not perform lookup/discovery during execute and it does not implement inventory state moves, bulk inventory, product updates, or location management.

`inventory.moveQuantity.execute` is the eighth limited real Shopify write tool. It only attempts `inventoryMoveQuantities` when read-only mode is disabled, local granted scopes include `write_inventory`, a stored `inventory.moveQuantity.preview` record exists, the preview is not expired, the reviewed payload hashes back to the stored preview, the target/tool/hash binding matches, and `confirmed: true` is present. It supports one explicit inventory item ID, one explicit location ID, one positive integer quantity, supported source and destination quantity names, an explicit reason, and optional reference document URI. It moves quantity between states at the same explicit location; it does not perform lookup/discovery during execute and it does not implement inter-location transfers, bulk inventory, product updates, or location management.

`inventory.transfer.execute` is the ninth limited real Shopify write tool. It only attempts `inventoryTransferCreate` when read-only mode is disabled, local granted scopes include both `write_inventory_transfers` and `read_inventory_transfers`, a stored `inventory.transfer.preview` record exists, the preview is not expired, the reviewed payload hashes back to the stored preview, the target/tool/hash binding matches, and `confirmed: true` is present. It supports one explicit inventory item ID, one source location ID, one destination location ID, one positive quantity, an explicit reason, and optional reference document URI. It creates a draft Shopify inventory transfer only; it does not mark transfers ready to ship, ship/receive/cancel transfers, perform lookup/discovery during execute, implement bulk inventory, or manage locations.

`inventory.transfer.addItems.execute` is the tenth limited real Shopify write tool. It only attempts `inventoryTransferSetItems` when read-only mode is disabled, local granted scopes include `write_inventory_transfers`, `read_inventory_transfers`, and `read_inventory`, a stored `inventory.transfer.addItems.preview` record exists, the preview is not expired, the reviewed payload hashes back to the stored preview, the target/tool/hash binding matches, and `confirmed: true` is present. It supports one explicit inventory transfer ID, one explicit inventory item ID, and one positive quantity only. It sets or replaces that one transfer item quantity; it does not create transfers, remove line items, mark ready, ship, receive, cancel, perform lookup/discovery during execute, implement bulk inventory, or manage locations.

`inventory.transfer.markReady.execute` is the eleventh limited real Shopify write tool. It only attempts `inventoryTransferMarkAsReadyToShip` when read-only mode is disabled, local granted scopes include both `write_inventory_transfers` and `read_inventory_transfers`, a stored `inventory.transfer.markReady.preview` record exists, the preview is not expired, the reviewed payload hashes back to the stored preview, the target/tool/hash binding matches, and `confirmed: true` is present. It supports one explicit inventory transfer ID only. It does not create transfers, edit line items, ship, receive, cancel, perform lookup/discovery during execute, implement bulk inventory, or manage locations.

`inventory.transfer.cancel.execute` is the twelfth limited real Shopify write tool. It only attempts `inventoryTransferCancel` when read-only mode is disabled, local granted scopes include both `write_inventory_transfers` and `read_inventory_transfers`, a stored `inventory.transfer.cancel.preview` record exists, the preview is not expired, the reviewed payload hashes back to the stored preview, the target/tool/hash binding matches, and `confirmed: true` is present. It supports one explicit inventory transfer ID only. It does not create transfers, edit line items, mark ready, ship, receive, perform lookup/discovery during execute, implement bulk inventory, or manage locations.

`inventory.transfer.ship.execute` is the thirteenth limited real Shopify write tool. It only attempts `inventoryShipmentCreateInTransit` when read-only mode is disabled, local granted scopes include both `write_inventory_shipments` and `read_inventory_shipments`, a stored `inventory.transfer.ship.preview` record exists, the preview is not expired, the reviewed payload hashes back to the stored preview, the target/tool/hash binding matches, and `confirmed: true` is present. It supports one explicit inventory transfer ID, one explicit inventory item ID, and one positive quantity only. It does not create transfers, edit transfer items, mark ready, cancel, receive shipments, perform lookup/discovery during execute, implement bulk inventory, or manage locations.

`inventory.transfer.receive.execute` is the fourteenth limited real Shopify write tool. It only attempts `inventoryShipmentReceive` when read-only mode is disabled, local granted scopes include both `write_inventory_shipments_received_items` and `read_inventory_shipments`, a stored `inventory.transfer.receive.preview` record exists, the preview is not expired, the reviewed payload hashes back to the stored preview, the target/tool/hash binding matches, and `confirmed: true` is present. It supports one explicit inventory shipment ID, one explicit shipment line item ID, one positive quantity, and a reviewed receive reason of `ACCEPTED` or `REJECTED` only. It does not create transfers, edit transfer items, mark ready, cancel, create shipments, perform lookup/discovery during execute, implement bulk inventory, or manage locations.

All other write tools are still guarded fail-closed placeholders, including product import execute, customer address update execute, refund execute, tracking update execute, bulk execute, and theme apply. Confirmation alone is insufficient: callers must provide a `previewId` plus reviewed payload context that can be tied to the expected preview tool, target, preview hash, and reviewed changes hash. Missing, expired, or mismatched preview binding fails closed and is audited as `blocked`; validly bound placeholders are audited as `not_implemented`, never `success`.
