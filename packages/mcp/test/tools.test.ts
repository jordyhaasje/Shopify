...
describe("MCP tools", () => {
  ...
  it("previews inventory quantity adjustment with executeRequest binding", async () => {
    const context = baseContext();

    const result = await callTool("inventory.adjustQuantity.preview", {
      inventoryItemId: "gid://shopify/InventoryItem/1",
      locationId: "gid://shopify/Location/1",
      delta: -2,
      reason: "correction"
    ...
    expect(result).toMatchObject({
      ...
      target: { type: "inventory", id: "gid://shopify/InventoryItem/1" },
      executeRequest: expect.objectContaining({
        tool: "inventory.adjustQuantity.execute",
        expectedTool: "inventory.adjustQuantity.preview",
        requiresConfirmation: true
      })
    });
    expect(changeFor(result, "delta")).toMatchObject({ before: "current available quantity", after: -2 });
    expect(context.audit.list()[0]).toMatchObject({ tool: "inventory.adjustQuantity.preview", mode: "preview", result: "success" });
  });

  it("adjusts inventory quantity from stored preview via inventoryAdjustQuantities", async () => {
    const requests: Array<{ body: string }> = [];
    const context = baseContext(async (_url, init) => {
      requests.push({ body: init.body });
      return jsonResponse({
        data: {
          inventoryAdjustQuantities: {
            inventoryAdjustmentGroup: {
              ...
              changes: [{ name: "available", delta: -2, quantityAfterChange: 6 }],
              rawNodeOnly: "do not return"
            },
            userErrors: []
          }
    ...
    context.config.grantedScopes = ["write_inventory"];
    const preview = await callTool("inventory.adjustQuantity.preview", {
      inventoryItemId: "gid://shopify/InventoryItem/1",
      locationId: "gid://shopify/Location/1",
      delta: -2,
      reason: "correction",
    ...
    const result = await callTool("inventory.adjustQuantity.execute", {
      previewId: preview.previewId,
      confirmed: true,
      reviewedPayload: reviewed.reviewedPayload,
      expectedTool: binding.expectedTool,
    ...
    expect(requests[0].body).not.toContain("999");
    expect(output).not.toContain("rawNodeOnly");
    expect(context.audit.list()[1]).toMatchObject({ tool: "inventory.adjustQuantity.execute", result: "success" });
  });

  it("blocks inventory adjustment before fetch when write_inventory is missing", async () => {
    let fetchCalled = false;
    const context = baseContext(async () => {
      fetchCalled = true;
      return jsonResponse({});
  ...
  it("previews inventory transfer add-items without executeRequest or fetch", async () => {
    ...
    const result = await callTool("inventory.transfer.addItems.preview", {
    ...
    expect(changeFor(result, "transferItems")).toMatchObject({ before: "DRAFT", after: "ADD_ITEM" });
  ...
  it("previews inventory transfer cancel with executeRequest binding", async () => {
    const context = baseContext();

    const result = await callTool("inventory.transfer.cancel.preview", {
      inventoryTransferId: "gid://shopify/InventoryTransfer/1",
    ...
    expect(result).toMatchObject({
      ...
      target: { type: "inventory_transfer", id: "gid://shopify/InventoryTransfer/1" },
      executeRequest: expect.objectContaining({
        tool: "inventory.transfer.cancel.execute",
        expectedTool: "inventory.transfer.cancel.preview",
        requiresConfirmation: true
      })
    });
    expect(result.warnings).toEqual([]);
    expect(changeFor(result, "status")).toMatchObject({ before: "READY_TO_SHIP", after: "CANCELLED" });
    expect(context.audit.list()[0]).toMatchObject({ tool: "inventory.transfer.cancel.preview", mode: "preview", result: "success" });
  });

  it("cancels inventory transfer from stored preview", async () => {
    const requests: Array<{ body: string }> = [];
    const context = baseContext(async (_url, init) => {
      requests.push({ body: init.body });
      return jsonResponse({
    ...
    context.config.grantedScopes = ["write_inventory_transfers", "read_inventory_transfers"];
    const preview = await callTool("inventory.transfer.cancel.preview", {
      inventoryTransferId: "gid://shopify/InventoryTransfer/1",
      currentStatus: "READY_TO_SHIP"
    }, context) as Record<string, unknown>;
    const binding = preview.binding as Record<string, unknown>;
    ...
    const result = await callTool("inventory.transfer.cancel.execute", {
      previewId: preview.previewId,
      confirmed: true,
      reviewedPayload: reviewed.reviewedPayload,
      expectedTool: binding.expectedTool,
    ...
    expect(requests[0].body).not.toContain("InventoryTransfer/999");
    expect(output).not.toContain("rawNodeOnly");
    expect(context.audit.list()[1]).toMatchObject({ tool: "inventory.transfer.cancel.execute", result: "success" });
  });

  it("blocks inventory transfer cancel before fetch when transfer scopes are missing", async () => {
    let fetchCalled = false;
    const context = baseContext(async () => {
      fetchCalled = true;
      return jsonResponse({});
    ...
    expect(fetchCalled).toBe(false);
    expect(context.audit.list()[1]).toMatchObject({ tool: "inventory.transfer.cancel.execute", result: "blocked" });
  });

  it("previews inventory transfer ship with executeRequest binding", async () => {
    const context = baseContext();

    const result = await callTool("inventory.transfer.ship.preview", {
      inventoryTransferId: "gid://shopify/InventoryTransfer/1",
      inventoryItemId: "gid://shopify/InventoryItem/1",
      quantity: 3,
      currentStatus: "READY_TO_SHIP"
    ...
    expect(result).toMatchObject({
      ...
      target: { type: "inventory_transfer", id: "gid://shopify/InventoryTransfer/1" },
      executeRequest: expect.objectContaining({
        tool: "inventory.transfer.ship.execute",
        expectedTool: "inventory.transfer.ship.preview",
        requiresConfirmation: true
      })
    });
    expect(result.warnings).toEqual([]);
    expect(changeFor(result, "quantity")).toMatchObject({ before: "READY_TO_SHIP", after: "IN_TRANSIT" });
    expect(context.audit.list()[0]).toMatchObject({ tool: "inventory.transfer.ship.preview", mode: "preview", result: "success" });
  });

  it("ships inventory transfer from stored preview", async () => {
    const requests: Array<{ body: string }> = [];
    const context = baseContext(async (_url, init) => {
      requests.push({ body: init.body });
      return jsonResponse({
    ...
    const preview = await callTool("inventory.transfer.ship.preview", {
      inventoryTransferId: "gid://shopify/InventoryTransfer/1",
      inventoryItemId: "gid://shopify/InventoryItem/1",
      quantity: 3,
      currentStatus: "READY_TO_SHIP"
    ...
    const result = await callTool("inventory.transfer.ship.execute", {
      previewId: preview.previewId,
      confirmed: true,
      reviewedPayload: reviewed.reviewedPayload,
      expectedTool: binding.expectedTool,
    ...
    expect(output).not.toContain("rawNodeOnly");
    expect(context.audit.list()[1]).toMatchObject({ tool: "inventory.transfer.ship.execute", result: "success" });
  });

  it("blocks inventory transfer ship before fetch when shipment scopes are missing", async () => {
    let fetchCalled = false;
    const context = baseContext(async () => {
      fetchCalled = true;
      return jsonResponse({});
  ...
  it("deletes explicit product options from stored product update preview via productOptionsDelete", async () => {
    const requests: Array<{ body: string }> = [];
    const context = baseContext(async (_url, init) => {
      requests.push({ body: init.body });
      return jsonResponse({
    ...
    const result = await callTool("product.update.execute", {
      ...
      previewHash: preview.previewHash,
      reviewedChangesHash: reviewed.reviewedChangesHash,
      deleteOptionIds: ["gid://shopify/ProductOption/999"]
    }, context);
    const request = JSON.parse(requests[0].body);
    const output = JSON.stringify(result);
  ...
  it("renames an explicit product option from stored product update preview via productOptionUpdate", async () => {
    ...
    const result = await callTool("product.update.execute", {
      ...
      previewHash: preview.previewHash,
      reviewedChangesHash: reviewed.reviewedChangesHash,
      options: [{ id: "gid://shopify/ProductOption/1", name: "Untrusted" }]
    }, context);
    const request = JSON.parse(requests[0].body);
    const output = JSON.stringify(result);
  ...
  it("renames an explicit product option value from stored product update preview via productOptionUpdate", async () => {
    const requests: Array<{ body: string }> = [];
    const context = baseContext(async (_url, init) => {
      requests.push({ body: init.body });
      return jsonResponse({
              ...
              options: [
                {
                  ...
                  position: 1,
                  optionValues: [
                    { id: "gid://shopify/ProductOptionValue/1", name: "Purple", hasVariants: true }
                  ],
                  rawNodeOnly: "do-not-return"
                }
              ],
    ...
    const result = await callTool("product.update.execute", {
      ...
      previewHash: preview.previewHash,
      reviewedChangesHash: reviewed.reviewedChangesHash,
      options: [{ id: "gid://shopify/ProductOption/1", optionValues: [{ id: "gid://shopify/ProductOptionValue/1", name: "Untrusted" }] }]
    }, context);
    const request = JSON.parse(requests[0].body);
    const output = JSON.stringify(result);
  ...
  it("deletes explicit product option values from stored product update preview via productOptionUpdate", async () => {
    const requests: Array<{ body: string }> = [];
    const context = baseContext(async (_url, init) => {
      requests.push({ body: init.body });
      return jsonResponse({
    ...
    const preview = await callTool("product.update.preview", {
      ...
      options: [{
        id: "gid://shopify/ProductOption/1",
        deleteValueIds: ["gid://shopify/ProductOptionValue/1"]
      }]
    }, context) as Record<string, unknown>;
    const binding = preview.binding as Record<string, unknown>;
    const reviewed = reviewedBindingFor(context, preview);
    ...
    const result = await callTool("product.update.execute", {
      ...
      previewHash: preview.previewHash,
      reviewedChangesHash: reviewed.reviewedChangesHash,
      options: [{ id: "gid://shopify/ProductOption/1", deleteValueIds: ["gid://shopify/ProductOptionValue/999"] }]
    }, context);
    const request = JSON.parse(requests[0].body);
    const output = JSON.stringify(result);
  ...
  it("blocks mixed basic product and option delete preview before fetch", async () => {
    let fetchCalled = false;
    const context = baseContext(async () => {
      fetchCalled = true;
      return jsonResponse({});
  ...
  it("blocks mixed basic product and option rename preview before fetch", async () => {
    let fetchCalled = false;
    const context = baseContext(async () => {
      fetchCalled = true;
      return jsonResponse({});
    ...
    const preview = await callTool("product.update.preview", {
      productId: "gid://shopify/Product/1",
      title: "Mixed Option Rename",
      options: [{ id: "gid://shopify/ProductOption/1", name: "Fabric" }]
    }, context) as Record<string, unknown>;
    const binding = preview.binding as Record<string, unknown>;
    const reviewed = reviewedBindingFor(context, preview);
  ...
  it("blocks mixed basic product and option value rename preview before fetch", async () => {
    let fetchCalled = false;
    const context = baseContext(async () => {
      fetchCalled = true;
      return jsonResponse({});
    ...
    const preview = await callTool("product.update.preview", {
      productId: "gid://shopify/Product/1",
      title: "Mixed Option Value Rename",
      options: [{ id: "gid://shopify/ProductOption/1", optionValues: [{ id: "gid://shopify/ProductOptionValue/1", name: "Purple" }] }]
    }, context) as Record<string, unknown>;
    const binding = preview.binding as Record<string, unknown>;
    const reviewed = reviewedBindingFor(context, preview);
  ...
  it("blocks mixed basic product and option value delete preview before fetch", async () => {
    let fetchCalled = false;
    const context = baseContext(async () => {
      fetchCalled = true;
      return jsonResponse({});
    ...
    const preview = await callTool("product.update.preview", {
      productId: "gid://shopify/Product/1",
      title: "Mixed Option Value Delete",
      options: [{ id: "gid://shopify/ProductOption/1", deleteValueIds: ["gid://shopify/ProductOptionValue/1"] }]
    }, context) as Record<string, unknown>;
    const binding = preview.binding as Record<string, unknown>;
    const reviewed = reviewedBindingFor(context, preview);
  ...
  it("blocks multiple option renames before fetch", async () => {
    ...
    const preview = await callTool("product.update.preview", {
      productId: "gid://shopify/Product/1",
      options: [
        { id: "gid://shopify/ProductOption/1", name: "Fabric" },
        { id: "gid://shopify/ProductOption/2", name: "Fit" }
      ]
    }, context) as Record<string, unknown>;
    const binding = preview.binding as Record<string, unknown>;
    const reviewed = reviewedBindingFor(context, preview);
    ...
    expect(result).toMatchObject({
      ...
      implemented: true,
      status: "blocked",
      diagnostics: expect.arrayContaining([expect.objectContaining({ code: "multiple_option_renames" })])
    });
    expect(fetchCalled).toBe(false);
    expect(context.audit.list()[1]).toMatchObject({ tool: "product.update.execute", result: "blocked" });
  });

  it("blocks multiple option value renames before fetch", async () => {
    let fetchCalled = false;
    const context = baseContext(async () => {
      fetchCalled = true;
      return jsonResponse({});
    ...
    const preview = await callTool("product.update.preview", {
      ...
      options: [{
        id: "gid://shopify/ProductOption/1",
        optionValues: [
          { id: "gid://shopify/ProductOptionValue/1", name: "Purple" },
          { id: "gid://shopify/ProductOptionValue/2", name: "Green" }
        ]
      }]
    }, context) as Record<string, unknown>;
    const binding = preview.binding as Record<string, unknown>;
  ...
  it("blocks option value deletes for multiple options before fetch", async () => {
    let fetchCalled = false;
    const context = baseContext(async () => {
      fetchCalled = true;
      return jsonResponse({});
    ...
    const preview = await callTool("product.update.preview", {
      productId: "gid://shopify/Product/1",
      options: [
        { id: "gid://shopify/ProductOption/1", deleteValueIds: ["gid://shopify/ProductOptionValue/1"] },
        { id: "gid://shopify/ProductOption/2", deleteValueIds: ["gid://shopify/ProductOptionValue/2"] }
      ]
    }, context) as Record<string, unknown>;
    const binding = preview.binding as Record<string, unknown>;
    const reviewed = reviewedBindingFor(context, preview);
    ...
    expect(result).toMatchObject({
      ...
      implemented: true,
      status: "blocked",
      diagnostics: expect.arrayContaining([expect.objectContaining({ code: "multiple_option_value_deletes" })])
    });
    expect(fetchCalled).toBe(false);
    expect(context.audit.list()[1]).toMatchObject({ tool: "product.update.execute", result: "blocked" });
  });
