import {
  ...
  type InventoryTransferShipInput,
  type InventoryTransferShipResult,
  type InventoryTransferReceiveInput,
  type InventoryTransferReceiveResult,
  loadStoredConfig,
  defaultAuditLogPath,
...
async function inventoryAdjustQuantityExecuteResult(input: Record<string, unknown>, context: ToolContext): Promise<Record<string, unknown>> {
  const tool = "inventory.adjustQuantity.execute";
  const target = inventoryExecuteTarget(input, context);

  if (context.config.readOnly) {
    return blockedImplementedExecuteResult(tool, target, [diagnostic("read_only", "Execute is blocked because read-only mode is enabled.")], context, "inventory.adjustQuantity.preview");
  }

  if (!context.previewStore) {
    return blockedImplementedExecuteResult(tool, target, [diagnostic("stored_preview_missing", "Stored preview was not found; execute binding fails closed.")], context, "inventory.adjustQuantity.preview");
  }
  ...
  const binding = verifyStoredPreviewBinding(context.previewStore, input, {
    executeTool: tool,
    expectedPreviewTool: "inventory.adjustQuantity.preview",
    target: storedTarget
  });
  if (!stringInput(input, "target")) {
    binding.ok = false;
  ...
  if (lookup.record.tool !== "inventory.adjustQuantity.preview") {
    return blockedImplementedExecuteResult(tool, storedTarget, [diagnostic("stored_preview_tool_mismatch", "Stored preview is not an inventory adjustment preview.")], context, binding);
  }

  const extracted = inventoryAdjustQuantityInputFromStoredPreview(lookup.record);
  if (!extracted.ok) return blockedImplementedExecuteResult(tool, storedTarget, extracted.diagnostics, context, binding);

  const preflight = checkWriteScopePreflight(context.config, tool);
  if (!preflight.ok) return blockedImplementedExecuteResult(tool, storedTarget, preflight.diagnostics, context, binding);

  const write = await adjustInventoryQuantity(context.config, extracted.input, { fetcher: context.fetcher });
  return inventoryAdjustQuantityWriteResult(tool, storedTarget, binding, write, context);
}

async function inventoryMoveQuantityExecuteResult(input: Record<string, unknown>, context: ToolContext): Promise<Record<string, unknown>> {
  const tool = "inventory.moveQuantity.execute";
  const target = inventoryExecuteTarget(input, context);

  if (context.config.readOnly) {
    return blockedImplementedExecuteResult(tool, target, [diagnostic("read_only", "Execute is blocked because read-only mode is enabled.")], context, "inventory.moveQuantity.preview");
  }

  if (!context.previewStore) {
    return blockedImplementedExecuteResult(tool, target, [diagnostic("stored_preview_missing", "Stored preview was not found; execute binding fails closed.")], context, "inventory.moveQuantity.preview");
  }
  ...
  const binding = verifyStoredPreviewBinding(context.previewStore, input, {
    executeTool: tool,
    expectedPreviewTool: "inventory.moveQuantity.preview",
    target: storedTarget
  });
  if (!stringInput(input, "target")) {
    binding.ok = false;
  ...
  if (lookup.record.tool !== "inventory.moveQuantity.preview") {
    return blockedImplementedExecuteResult(tool, storedTarget, [diagnostic("stored_preview_tool_mismatch", "Stored preview is not an inventory move preview.")], context, binding);
  }

  const extracted = inventoryMoveQuantityInputFromStoredPreview(lookup.record);
  if (!extracted.ok) return blockedImplementedExecuteResult(tool, storedTarget, extracted.diagnostics, context, binding);
...
async function inventoryTransferMarkReadyExecuteResult(input: Record<string, unknown>, context: ToolContext): Promise<Record<string, unknown>> {
  const tool = "inventory.transfer.markReady.execute";
  const target = inventoryExecuteTarget(input, context);

  if (context.config.readOnly) {
    return blockedImplementedExecuteResult(tool, target, [diagnostic("read_only", "Execute is blocked because read-only mode is enabled.")], context, "inventory.transfer.markReady.preview");
  }

  if (!context.previewStore) {
    return blockedImplementedExecuteResult(tool, target, [diagnostic("stored_preview_missing", "Stored preview was not found; execute binding fails closed.")], context, "inventory.transfer.markReady.preview");
  }
  ...
  const binding = verifyStoredPreviewBinding(context.previewStore, input, {
    executeTool: tool,
    expectedPreviewTool: "inventory.transfer.markReady.preview",
    target: storedTarget
  });
  if (!stringInput(input, "target")) {
    binding.ok = false;
  ...
  if (lookup.record.tool !== "inventory.transfer.markReady.preview") {
    return blockedImplementedExecuteResult(tool, storedTarget, [diagnostic("stored_preview_tool_mismatch", "Stored preview is not an inventory transfer mark-ready preview.")], context, binding);
  }

  const extracted = inventoryTransferMarkReadyInputFromStoredPreview(lookup.record);
  if (!extracted.ok) return blockedImplementedExecuteResult(tool, storedTarget, extracted.diagnostics, context, binding);
...
async function inventoryTransferShipExecuteResult(input: Record<string, unknown>, context: ToolContext): Promise<Record<string, unknown>> {
  const tool = "inventory.transfer.ship.execute";
  const target = inventoryExecuteTarget(input, context);

  if (context.config.readOnly) {
    return blockedImplementedExecuteResult(tool, target, [diagnostic("read_only", "Execute is blocked because read-only mode is enabled.")], context, "inventory.transfer.ship.preview");
  }

  if (!context.previewStore) {
    return blockedImplementedExecuteResult(tool, target, [diagnostic("stored_preview_missing", "Stored preview was not found; execute binding fails closed.")], context, "inventory.transfer.ship.preview");
  }
  ...
  const binding = verifyStoredPreviewBinding(context.previewStore, input, {
    executeTool: tool,
    expectedPreviewTool: "inventory.transfer.ship.preview",
    target: storedTarget
  });
  if (!stringInput(input, "target")) {
    binding.ok = false;
  ...
  if (lookup.record.tool !== "inventory.transfer.ship.preview") {
    return blockedImplementedExecuteResult(tool, storedTarget, [diagnostic("stored_preview_tool_mismatch", "Stored preview is not an inventory transfer ship preview.")], context, binding);
  }

  const extracted = inventoryTransferShipInputFromStoredPreview(lookup.record);
  if (!extracted.ok) return blockedImplementedExecuteResult(tool, storedTarget, extracted.diagnostics, context, binding);

  const preflight = checkWriteScopePreflight(context.config, tool);
  if (!preflight.ok) return blockedImplementedExecuteResult(tool, storedTarget, preflight.diagnostics, context, binding);

  const write = await shipInventoryTransfer(context.config, extracted.input, { fetcher: context.fetcher });
  return inventoryTransferShipWriteResult(tool, storedTarget, binding, write, context);
}

async function inventoryTransferReceiveExecuteResult(input: Record<string, unknown>, context: ToolContext): Promise<Record<string, unknown>> {
  const tool = "inventory.transfer.receive.execute";
  const target = inventoryExecuteTarget(input, context);

  if (context.config.readOnly) {
    return blockedImplementedExecuteResult(tool, target, [diagnostic("read_only", "Execute is blocked because read-only mode is enabled.")], context, "inventory.transfer.receive.preview");
  }

  if (!context.previewStore) {
    return blockedImplementedExecuteResult(tool, target, [diagnostic("stored_preview_missing", "Stored preview was not found; execute binding fails closed.")], context, "inventory.transfer.receive.preview");
  }
  ...
  const binding = verifyStoredPreviewBinding(context.previewStore, input, {
    executeTool: tool,
    expectedPreviewTool: "inventory.transfer.receive.preview",
    target: storedTarget
  });
  if (!stringInput(input, "target")) {
    binding.ok = false;
  ...
  if (lookup.record.tool !== "inventory.transfer.receive.preview") {
    return blockedImplementedExecuteResult(tool, storedTarget, [diagnostic("stored_preview_tool_mismatch", "Stored preview is not an inventory transfer receive preview.")], context, binding);
  }

  const extracted = inventoryTransferReceiveInputFromStoredPreview(lookup.record);
  if (!extracted.ok) return blockedImplementedExecuteResult(tool, storedTarget, extracted.diagnostics, context, binding);

  const preflight = checkWriteScopePreflight(context.config, tool);
  if (!preflight.ok) return blockedImplementedExecuteResult(tool, storedTarget, preflight.diagnostics, context, binding);

  const write = await receiveInventoryTransfer(context.config, extracted.input, { fetcher: context.fetcher });
  return inventoryTransferReceiveWriteResult(tool, storedTarget, binding, write, context);
}
...
function inventoryAdjustQuantityWriteResult(
  tool: string,
  target: string,
  binding: ExecutePreviewBindingResult,
  write: InventoryAdjustQuantityResult,
  context: ToolContext
): Record<string, unknown> {
  const result = inventoryWriteAuditResult(write);
  const audit = context.audit.record({
  ...
  return {
    ...
    summary: write.summary,
    audit,
    inventoryAdjustment: write.inventoryAdjustment,
    userErrors: write.userErrors,
    diagnostics: write.diagnostics,
    previewBinding: {
      previewId: binding.previewId,
...
function inventoryMoveQuantityWriteResult(
  tool: string,
  target: string,
  binding: ExecutePreviewBindingResult,
  write: InventoryMoveQuantityResult,
  context: ToolContext
): Record<string, unknown> {
  const result = inventoryWriteAuditResult(write);
  const audit = context.audit.record({
  ...
  return {
    ...
    summary: write.summary,
    audit,
    inventoryMove: write.inventoryMove,
    userErrors: write.userErrors,
    diagnostics: write.diagnostics,
    previewBinding: {
      previewId: binding.previewId,
...
function inventoryTransferMarkReadyWriteResult(
  ...
  target: string,
  binding: ExecutePreviewBindingResult,
  write: InventoryTransferMarkReadyResult,
  context: ToolContext
): Record<string, unknown> {
  const result = inventoryWriteAuditResult(write);
  const audit = context.audit.record({
...
function inventoryTransferShipWriteResult(
  tool: string,
  target: string,
  binding: ExecutePreviewBindingResult,
  write: InventoryTransferShipResult,
  context: ToolContext
): Record<string, unknown> {
  const result = inventoryWriteAuditResult(write);
  const audit = context.audit.record({
  ...
  return {
    ...
    summary: write.summary,
    audit,
    inventoryShipment: write.inventoryShipment,
    userErrors: write.userErrors,
    diagnostics: write.diagnostics,
    previewBinding: {
      previewId: binding.previewId,
...
function inventoryTransferReceiveWriteResult(
  tool: string,
  target: string,
  binding: ExecutePreviewBindingResult,
  write: InventoryTransferReceiveResult,
  context: ToolContext
): Record<string, unknown> {
  const result = inventoryWriteAuditResult(write);
  const audit = context.audit.record({
...
function inventoryAdjustQuantityInputFromStoredPreview(record: StoredPreviewRecord): { ok: true; input: InventoryAdjustQuantityInput } | { ok: false; diagnostics: ExecutePreviewBindingDiagnostic[] } {
  const inventoryItemId = safeContentText(targetField(record, "id") ?? changeValue(record, "inventoryItemId"), 180);
  const locationId = safeContentText(changeValue(record, "locationId"), 180);
  const delta = safeInteger(changeAfterValue(record, "delta"));
  const reason = safeContentText(changeValue(record, "reason"), 120);
  const referenceDocumentUri = safeContentText(changeValue(record, "referenceDocumentUri"), 255);
  ...
  if (!inventoryItemId) {
    return {
      ok: false,
      diagnostics: [diagnostic("missing_inventory_item_id", "Stored inventory adjustment preview did not include a safe inventory item ID.")]
    };
  }
  if (!locationId) {
    return {
      ok: false,
      diagnostics: [diagnostic("missing_inventory_location_id", "Stored inventory adjustment preview did not include a safe location ID.")]
    };
  }
  if (delta === undefined || delta === 0) {
    return {
      ok: false,
      diagnostics: [diagnostic("missing_inventory_delta", "Stored inventory adjustment preview did not include a safe non-zero delta.")]
    };
  }
  if (!reason) {
    return {
      ok: false,
      diagnostics: [diagnostic("missing_inventory_reason", "Stored inventory adjustment preview did not include a safe inventory adjustment reason.")]
    };
  }
...
function inventoryMoveQuantityInputFromStoredPreview(record: StoredPreviewRecord): { ok: true; input: InventoryMoveQuantityInput } | { ok: false; diagnostics: ExecutePreviewBindingDiagnostic[] } {
  const inventoryItemId = safeContentText(targetField(record, "id") ?? changeValue(record, "inventoryItemId"), 180);
  const locationId = safeContentText(changeValue(record, "locationId"), 180);
  const quantity = safeInteger(changeAfterValue(record, "quantity"));
  const fromName = inventoryQuantityNameFromStoredPreview(changeValue(record, "fromName"));
  const toName = inventoryQuantityNameFromStoredPreview(changeValue(record, "toName"));
  const reason = safeContentText(changeValue(record, "reason"), 120);
  const referenceDocumentUri = safeContentText(changeValue(record, "referenceDocumentUri"), 255);
  ...
  if (!inventoryItemId) {
    return {
      ok: false,
      diagnostics: [diagnostic("missing_inventory_item_id", "Stored inventory move preview did not include a safe inventory item ID.")]
    };
  }
  if (!locationId) {
    return {
      ok: false,
      diagnostics: [diagnostic("missing_inventory_location_id", "Stored inventory move preview did not include a safe location ID.")]
    };
  }
  if (quantity === undefined || quantity <= 0) {
    return {
      ok: false,
      diagnostics: [diagnostic("missing_inventory_move_quantity", "Stored inventory move preview did not include a safe positive quantity.")]
    };
  }
  if (!fromName || !toName || fromName === toName) {
    return {
      ok: false,
      diagnostics: [diagnostic("missing_inventory_move_names", "Stored inventory move preview did not include safe distinct quantity names.")]
    };
  }
  if (!reason) {
    return {
      ok: false,
      diagnostics: [diagnostic("missing_inventory_reason", "Stored inventory move preview did not include a safe inventory move reason.")]
    };
  }
...
function inventoryTransferMarkReadyInputFromStoredPreview(record: StoredPreviewRecord): { ok: true; input: InventoryTransferMarkReadyInput } | { ok: false; diagnostics: ExecutePreviewBindingDiagnostic[] } {
  ...
  if (!inventoryTransferId) {
    return {
      ok: false,
      diagnostics: [diagnostic("missing_inventory_transfer_id", "Stored inventory transfer mark-ready preview did not include a safe inventory transfer ID.")]
    };
  }
...
function inventoryTransferShipInputFromStoredPreview(record: StoredPreviewRecord): { ok: true; input: InventoryTransferShipInput } | { ok: false; diagnostics: ExecutePreviewBindingDiagnostic[] } {
  const inventoryTransferId = safeContentText(targetField(record, "id") ?? changeValue(record, "inventoryTransferId"), 180);
  const inventoryItemId = safeContentText(changeValue(record, "inventoryItemId"), 180);
  const quantity = safeInteger(changeValue(record, "quantityValue"));
  ...
  if (!inventoryTransferId) {
    return {
      ok: false,
      diagnostics: [diagnostic("missing_inventory_transfer_id", "Stored inventory transfer ship preview did not include a safe inventory transfer ID.")]
    };
  }
  if (!inventoryItemId) {
    return {
      ok: false,
      diagnostics: [diagnostic("missing_inventory_item_id", "Stored inventory transfer ship preview did not include a safe inventory item ID.")]
    };
  }
  if (quantity === undefined || quantity <= 0) {
    return {
      ok: false,
      diagnostics: [diagnostic("missing_inventory_transfer_ship_quantity", "Stored inventory transfer ship preview did not include a safe positive quantity.")]
    };
  }
...
function inventoryTransferReceiveInputFromStoredPreview(record: StoredPreviewRecord): { ok: true; input: InventoryTransferReceiveInput } | { ok: false; diagnostics: ExecutePreviewBindingDiagnostic[] } {
  const inventoryShipmentId = safeContentText(targetField(record, "id") ?? changeValue(record, "inventoryShipmentId"), 180);
  const shipmentLineItemId = safeContentText(changeValue(record, "shipmentLineItemId"), 180);
  const quantity = safeInteger(changeValue(record, "quantityValue"));
  const reason = receiveReasonFromStoredPreview(changeValue(record, "reason"));

  if (!inventoryShipmentId) {
    return {
      ok: false,
      diagnostics: [diagnostic("missing_inventory_shipment_id", "Stored inventory transfer receive preview did not include a safe inventory shipment ID.")]
    };
  }
  if (!shipmentLineItemId) {
    return {
      ok: false,
      diagnostics: [diagnostic("missing_inventory_shipment_line_item_id", "Stored inventory transfer receive preview did not include a safe inventory shipment line item ID.")]
    };
  }
  if (quantity === undefined || quantity <= 0) {
    return {
      ok: false,
      diagnostics: [diagnostic("missing_inventory_transfer_receive_quantity", "Stored inventory transfer receive preview did not include a safe positive quantity.")]
    };
  }
  if (!reason) {
    return {
      ok: false,
      diagnostics: [diagnostic("missing_inventory_transfer_receive_reason", "Stored inventory transfer receive preview did not include a safe receive reason.")]
    };
  }
...
function receiveReasonFromStoredPreview(value: unknown): "ACCEPTED" | "REJECTED" | undefined {
  const text = safeContentText(value, 80)?.toUpperCase();
...
export const tools: ToolDefinition[] = [
  ...
  {
    name: "inventory.adjustQuantity.preview",
    description: "Preview adjusting one explicit inventory item quantity delta at an explicit location.",
    inputSchema: { type: "object" },
    handler: (input, context) => catalogPreviewResult(previewInventoryAdjustQuantity(input), context)
  },
  {
    name: "inventory.adjustQuantity.execute",
    description: "Adjust one Shopify inventory item quantity delta only after stored preview binding and explicit confirmation.",
    inputSchema: { type: "object" },
    handler: (input, context) => inventoryAdjustQuantityExecuteResult(input, context)
  },
  {
    name: "inventory.moveQuantity.preview",
    description: "Preview moving inventory quantity between states for one explicit inventory item at one explicit location.",
    inputSchema: { type: "object" },
    handler: (input, context) => catalogPreviewResult(previewInventoryMoveQuantity(input), context)
  },
  {
    name: "inventory.moveQuantity.execute",
  ...
  },
  {
    ...
    description: "Preview marking one explicit Shopify inventory transfer ready to ship.",
    inputSchema: { type: "object" },
    handler: (input, context) => catalogPreviewResult(previewInventoryTransferMarkReady(input), context)
  },
  {
    name: "inventory.transfer.markReady.execute",
  ...
  {
    name: "inventory.transfer.ship.preview",
    description: "Preview shipping one explicit inventory item quantity from an explicit Shopify inventory transfer.",
    inputSchema: { type: "object" },
    handler: (input, context) => catalogPreviewResult(previewInventoryTransferShip(input), context)
  },
  {
    name: "inventory.transfer.ship.execute",
    description: "Create one in-transit inventory shipment only after stored preview binding and explicit confirmation.",
    inputSchema: { type: "object" },
    handler: (input, context) => inventoryTransferShipExecuteResult(input, context)
  },
  {
    name: "inventory.transfer.receive.preview",
    description: "Preview receiving one explicit inventory shipment line item quantity.",
    inputSchema: { type: "object" },
    handler: (input, context) => catalogPreviewResult(previewInventoryTransferReceive(input), context)
  },
  {
    name: "inventory.transfer.receive.execute",
    description: "Receive one Shopify inventory shipment line item only after stored preview binding and explicit confirmation.",
    inputSchema: { type: "object" },
    handler: (input, context) => inventoryTransferReceiveExecuteResult(input, context)
  },
  {
    name: "order.find",
