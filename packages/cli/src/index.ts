...
function setupNextSteps(authMethod: "manual" | "oauth", configPath: string): string[] {
  const common = [
    ...
    "Keep read-only mode enabled except for explicit reviewed development-store tests of page.create.execute, product.create.execute, product.update.execute, collection.create.execute, inventory.setQuantity.execute, inventory.adjustQuantity.execute, inventory.moveQuantity.execute, or inventory.transfer.execute."
  ];
  if (authMethod === "oauth") {
    return [
