...
function resolveSetupScopes(input: string | readonly string[] | undefined, readOnly: boolean): string[] {
  ...
  if (readOnly && writeScopes.length > 0) {
    throw new Error("write_scopes_blocked: Write scopes require --write-enabled. Only page.create.execute, product.create.execute, product.update.execute, collection.create.execute, and inventory.setQuantity.execute are implemented; all other execute tools remain fail-closed placeholders.");
  }
  return scopes;
}
...
function setupWarnings(readOnly: boolean, scopes: readonly string[]): SetupWarning[] {
  ...
  if (!readOnly) {
    warnings.push({
      ...
      message: "Write mode was requested. Only page.create.execute, product.create.execute, product.update.execute, collection.create.execute, and inventory.setQuantity.execute are implemented; all other execute tools remain fail-closed placeholders."
    });
  }
  if (scopes.some((scope) => scope.toLowerCase().startsWith("write_"))) {
    warnings.push({
      code: "write_scopes_requested",
      message: "Write scopes were requested. Use only the minimal scopes required for reviewed development-store tests of page.create.execute, product.create.execute, product.update.execute, collection.create.execute, or inventory.setQuantity.execute."
    });
  }
  return warnings;
}
...
function setupNextSteps(authMethod: "manual" | "oauth", configPath: string): string[] {
  const common = [
    ...
    "Keep read-only mode enabled except for explicit reviewed development-store tests of page.create.execute, product.create.execute, product.update.execute, collection.create.execute, or inventory.setQuantity.execute."
  ];
  if (authMethod === "oauth") {
    return [
