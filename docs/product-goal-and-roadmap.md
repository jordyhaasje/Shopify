...
- Preview tools are implemented for product, page, collection, import, media, inventory transfer, inventory transfer add-item planning, preview-only inventory transfer remove-item planning, and related planning surfaces.
...
- Generic `bulk.preview` is implemented for explicit batch-change inputs with safe summarized output, stored preview binding, and audit entries. It summarizes value types, string lengths, object keys, array counts, changed non-secret keys, and redaction markers, and does not return raw before/after payload values. `bulk.execute` remains a fail-closed placeholder.
- Minimal `product.media.update.execute` is implemented for add-only media previews: one explicit product ID, user-provided HTTP(S) media URLs or staged upload URLs, optional alt text, stored preview binding, and local `write_products` preflight. Media update/delete/reorder/replacement, staged upload orchestration, file library workflows, variant-media attachment, product discovery, and bulk media operations remain unimplemented.
