...
- `inventory.transfer.receive.execute` for one explicit inventory shipment ID, shipment line item ID, positive quantity, receive reason, and shipment-line-item receive only
- `product.media.update.execute` for one explicit product ID plus add-only user-provided HTTP(S) media URLs or staged upload URLs only
