...
Use write mode only for reviewed development-store tests of `page.create.execute`, `product.create.execute`, basic-field, explicit-variant-price, or explicit-variant-create `product.update.execute`, or custom explicit-product `collection.create.execute`. All other execute tools remain fail-closed placeholders.
...
- `product.update.execute` for basic product fields, explicit variant price updates, or explicit variant creation only
