...
Use write mode only for reviewed development-store tests of `page.create.execute`, `product.create.execute`, basic-field or explicit-variant-price `product.update.execute`, or custom explicit-product `collection.create.execute`. All other execute tools remain fail-closed placeholders.
...
- `product.update.execute` for basic product fields or explicit variant price updates only
