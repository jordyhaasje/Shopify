...
`product.create.preview`, `page.create.preview`, `collection.create.preview`, `inventory.setQuantity.preview`, and `inventory.adjustQuantity.preview` include an `executeRequest` helper. It contains the matching execute tool, expected preview tool, `previewId`, target, `previewHash`, safe reviewed payload, reviewed changes hash, and confirmation requirement. Use it to prepare the execute call for review, not to run automatically.
