...
Implemented write previews also return `executeRequest` as an AI-host UX helper. For `product.create.preview`, it points to `product.create.execute`; for `page.create.preview`, it points to `page.create.execute`; for `collection.create.preview`, it points to `collection.create.execute`; for `inventory.setQuantity.preview`, it points to `inventory.setQuantity.execute`; for `inventory.adjustQuantity.preview`, it points to `inventory.adjustQuantity.execute`; for `inventory.moveQuantity.preview`, it points to `inventory.moveQuantity.execute`. The helper contains the execute tool name, expected preview tool, preview ID, target, preview hash, safe reviewed payload, reviewed changes hash, and confirmation requirement. It is only a prepared request for review. It must not be treated as auto-execute, must not be submitted without explicit user approval, and does not bypass stored-preview validation.
