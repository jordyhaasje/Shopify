...
These are the intended V1 Shopify Store Agent MCP tool contracts. The current implementation includes real read-only Admin GraphQL tools, structured catalog/content preview tools, a local preview store, and six limited real write tools: `page.create.execute`, `product.create.execute`, `product.update.execute`, `collection.create.execute`, `inventory.setQuantity.execute`, and `inventory.adjustQuantity.execute`. All other execute/write tools remain fail-closed placeholders.
...
Implemented write previews also return `executeRequest` as an AI-host UX helper. For `product.create.preview`, it points to `product.create.execute`; for `page.create.preview`, it points to `page.create.execute`; for `collection.create.preview`, it points to `collection.create.execute`; for `inventory.setQuantity.preview`, it points to `inventory.setQuantity.execute`; for `inventory.adjustQuantity.preview`, it points to `inventory.adjustQuantity.execute`. The helper contains the execute tool name, expected preview tool, preview ID, target, preview hash, safe reviewed payload, reviewed changes hash, and confirmation requirement. It is only a prepared request for review. It must not be treated as auto-execute, must not be submitted without explicit user approval, and does not bypass stored-preview validation.
...
Not implemented here: execute-time lookup/discovery, location search by name, inventory moves, multi-item adjustments, bulk inventory, location management, or product update inventory fields.
...
Not implemented here: execute-time lookup/discovery, location search by name, inventory moves, bulk inventory, location management, or product update inventory fields.
...
Not implemented here: location search by name, product/handle browsing, inventory moves, bulk inventory, or location management.
