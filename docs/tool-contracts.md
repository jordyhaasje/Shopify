...
These are the intended V1 Shopify Store Agent MCP tool contracts. The current implementation includes real read-only Admin GraphQL tools, structured catalog/content preview tools, a local preview store, and three limited real write tools: `page.create.execute`, `product.create.execute`, and `product.update.execute`. All other execute/write tools remain fail-closed placeholders.
