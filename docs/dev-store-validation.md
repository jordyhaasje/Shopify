...
- Do not request write scopes for read, preview, setup, or smoke validation. For limited write tests, use only a development store and the minimum write scope needed for that execute path: `write_content` or `write_online_store_pages` for `page.create.execute`, `write_products` for `product.create.execute`, basic-field, explicit-variant-price, explicit-variant-create, explicit-option-create, explicit-option-delete, explicit-option-reorder, explicit-option-rename, explicit-option-value-rename, explicit-option-value-add, or explicit-option-value-delete `product.update.execute`, or custom explicit-product `collection.create.execute`, and `write_inventory` for `inventory.setQuantity.execute`, `inventory.adjustQuantity.execute`, or `inventory.moveQuantity.execute`. Read-only mode must be explicitly disabled for each deliberate write test.
...
- If testing `inventory.transfer.preview`, create it only with explicit inventory item ID, distinct source/destination location IDs, positive quantity, and reason. Confirm it stores a safe preview, returns no `executeRequest`, performs no write, and clearly reports that transfer execute is not implemented.
...
- Confirm all execute tools except `page.create.execute`, `product.create.execute`, `product.update.execute`, `collection.create.execute`, `inventory.setQuantity.execute`, `inventory.adjustQuantity.execute`, and `inventory.moveQuantity.execute` still remain placeholders.
...
- Confirm any deliberate write test is limited to `page.create.execute`, `product.create.execute`, basic-field, explicit-variant-price, explicit-variant-create, explicit-option-create, explicit-option-delete, explicit-option-reorder, explicit-option-rename, explicit-option-value-rename, explicit-option-value-add, or explicit-option-value-delete `product.update.execute`, custom explicit-product `collection.create.execute`, explicit single-item `inventory.setQuantity.execute`, explicit single-item `inventory.adjustQuantity.execute`, or explicit same-location state `inventory.moveQuantity.execute` in a development store.
...
- Keep read-only mode enabled except for explicit reviewed `page.create.execute`, `product.create.execute`, basic-field, explicit-variant-price, explicit-variant-create, explicit-option-create, explicit-option-delete, explicit-option-reorder, explicit-option-rename, explicit-option-value-rename, explicit-option-value-add, or explicit-option-value-delete `product.update.execute`, custom explicit-product `collection.create.execute`, explicit single-item `inventory.setQuantity.execute`, explicit single-item `inventory.adjustQuantity.execute`, or explicit same-location state `inventory.moveQuantity.execute` development-store tests.
