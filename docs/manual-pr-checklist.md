...
- For `product.update.execute` changes, confirm it is blocked by read-only mode, requires a valid stored `product.update.preview` binding, locally preflights `write_products`, calls Shopify only after binding and scope preflight succeed, performs only one supported mutation shape per preview (`productUpdate` for basic product fields, `productVariantsBulkUpdate` for explicit variant price updates, or `productVariantsBulkCreate` for explicit variant creation), blocks mixed update-shape previews before fetch, supports only title, description/descriptionHtml, vendor, product type, status, tags, explicit variant IDs with prices, or explicit variant option values with optional price/SKU from the stored preview, fails closed for handle-only previews without a safe product ID, returns a safe updated product, variant-price, or created-variant summary, audits success only after a successful Shopify write, and never returns raw reviewed payloads or raw Shopify nodes.
