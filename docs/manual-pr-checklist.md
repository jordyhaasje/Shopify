...
- For `product.update.execute` changes, confirm it is blocked by read-only mode, requires a valid stored `product.update.preview` binding, locally preflights `write_products`, calls Shopify only after binding and scope preflight succeed, performs only `productUpdate` for basic product fields or `productVariantsBulkUpdate` for explicit variant price updates, blocks mixed basic-field plus variant-price previews before fetch, supports only title, description/descriptionHtml, vendor, product type, status, tags, or explicit variant IDs with prices from the stored preview, fails closed for handle-only previews without a safe product ID, returns a safe updated product or variant-price summary, audits success only after a successful Shopify write, and never returns raw reviewed payloads or raw Shopify nodes.
