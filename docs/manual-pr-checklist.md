...
- For `product.update.execute` changes, confirm it is blocked by read-only mode, requires a valid stored `product.update.preview` binding, locally preflights `write_products`, calls Shopify only after binding and scope preflight succeed, performs only one supported mutation shape per preview (`productUpdate` for basic product fields, `productVariantsBulkUpdate` for explicit variant price updates, `productVariantsBulkCreate` for explicit variant creation, `productOptionsCreate` with `LEAVE_AS_IS` for explicit option creation, or `productOptionUpdate` with `LEAVE_AS_IS` for explicit option rename, option value rename, or option value add), blocks mixed update-shape previews before fetch, supports only title, description/descriptionHtml, vendor, product type, status, tags, explicit variant IDs with prices, explicit variant option values with optional price/SKU, explicit option names and values, explicit option ID plus new name, explicit option ID plus option value ID and new value name, or explicit option ID plus new value names from the stored preview, fails closed for handle-only previews without a safe product ID, returns a safe updated product, variant-price, created-variant, created-option, renamed-option, renamed-option-value, or added-option-values summary, audits success only after a successful Shopify write, and never returns raw reviewed payloads or raw Shopify nodes.
