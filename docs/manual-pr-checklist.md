...
- Confirm write-mode warnings name `page.create.execute`, `product.create.execute`, `product.update.execute`, `collection.create.execute`, and `inventory.setQuantity.execute` as the only implemented execute tools, with all other execute tools remaining fail-closed placeholders.
