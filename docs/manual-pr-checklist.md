...
- For `product.update.execute` changes, confirm it is blocked by read-only mode, requires a valid stored `product.update.preview` binding, locally preflights `write_products`, calls Shopify only after binding and scope preflight succeed, performs only the `productUpdate` mutation, supports only title, description/descriptionHtml, vendor, product type, status, and tags from the stored preview, fails closed for handle-only previews without a safe product ID, returns a safe updated product summary, audits success only after a successful product update, and never returns raw reviewed payloads or raw Shopify nodes.
