...
It also supports explicit variant price updates when the stored preview contains a product ID plus explicit variant IDs and non-negative prices, and explicit variant creation when the stored preview contains a product ID plus explicit option values with optional price/SKU. These use `productVariantsBulkUpdate` and `productVariantsBulkCreate` respectively, and must not be mixed with basic product field updates or each other in the same execute call.
...
Do not silently add media, broader product option management, SEO, inventory, metafields, publications, translations, or collection changes to the existing product update path.

Any future expansion of `product.update.execute` must be a separate roadmap item with focused tests and docs. The existing basic-field, explicit-variant-price, and explicit-variant-create mutations should remain easy to review and reason about.
