...
- `inventory.transfer.addItems.preview` is preview-only for one explicit inventory transfer ID, one explicit inventory item ID, and one positive quantity. It stores a safe preview binding and audit entry, does not call Shopify, and must not be treated as permission to execute a transfer item edit.
