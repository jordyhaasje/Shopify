...
`product.update.execute` also requires `write_products` in local granted-scope preflight. It is limited to either basic fields from a stored `product.update.preview` (title, description/descriptionHtml, vendor, product type, status, and tags) or explicit variant price updates with product ID, variant IDs, and prices. Mixed basic-field plus variant-price previews fail closed before fetch. Handle-only previews fail closed unless the stored preview contains a safe product ID. General variant creation/options, inventory, media/files/images, collections, metafields, SEO, publications, translations, deletes, and bulk operations remain outside this write path.
