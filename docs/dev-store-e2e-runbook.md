...
- `product.update.execute` for basic fields, explicit variant price updates, explicit variant creation, explicit option creation, explicit option rename, or explicit option value rename only.
...
For `product.create.execute`, basic-field, explicit-variant-price, explicit-variant-create, explicit-option-create, explicit-option-rename, or explicit-option-value-rename `product.update.execute`, or custom explicit-product `collection.create.execute`, add:
...
Write mode is only for reviewed development-store tests of `page.create.execute`, `product.create.execute`, basic-field, explicit-variant-price, explicit-variant-create, explicit-option-create, explicit-option-rename, or explicit-option-value-rename `product.update.execute`, or custom explicit-product `collection.create.execute`. All other execute tools remain fail-closed placeholders.
...
- No option value add/delete, option reorder/delete, inventory, media/files/images, collections, metafields, SEO bulk fields, publications/channels, translations, delete, or bulk operations are performed.
- Output and audit contain no secrets, raw reviewed payload, or full Shopify node.
